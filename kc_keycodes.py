...
customKeycodes declared in the VIA definition (QK_KB_0 + index). Nothing here is
guessed: writing any of these values is safe for this exact board.

Layout:
    CODE2LABEL : {int keycode -> short label}   (for rendering keys)
    GROUPS     : ordered [(group name, [codes])] (for the picker UI)
Custom Keychron keycodes are injected from the VIA JSON at load time so their
order/labels always match the definition file.
"""
import json, os

# ---- basic HID range (stable across QMK versions) -------------------------
...
# ---- system / media / brightness (QMK internal, from keycodes.h) ----------
CODE2LABEL.update({
...
# ---- white backlight (QK_BACKLIGHT_*, from keycodes.h) ---------------------
CODE2LABEL.update({
    0x7800: "BL On", 0x7801: "BL Off", 0x7802: "BL Tgl", 0x7803: "BL-",
    0x7804: "BL+", 0x7805: "BL Step", 0x7806: "BL Breath",
})

QK_KB_0 = 0x7E00   # base of Keychron customKeycodes (QK_KB range)

# ---- ordered groups for the picker ---------------------------------------
GROUPS = [
    ("Lettres",       [0x0004 + i for i in range(26)]),
    ("Chiffres",      [0x001E + i for i in range(10)]),
    ("Fonctions",     [0x003A + i for i in range(12)]),
    ("F13–F24",       [0x0068 + i for i in range(12)]),
    ("Navigation",    [0x0049,0x004A,0x004B,0x004C,0x004D,0x004E,0x004F,0x0050,0x0051,0x0052]),
    ("Modificateurs", [0x00E0,0x00E1,0x00E2,0x00E3,0x00E4,0x00E5,0x00E6,0x00E7]),
    ("Ponctuation",   [0x002D,0x002E,0x002F,0x0030,0x0031,0x0033,0x0034,0x0035,0x0036,0x0037,0x0038,0x0064]),
    ("Édition",       [0x0028,0x0029,0x002A,0x002B,0x002C,0x0039,0x0046,0x0065]),
    ("Média",         [0x00A8,0x00A9,0x00AA,0x00AB,0x00AC,0x00AD,0x00AE,0x00AF,0x00B0]),
    ("Système",       [0x00A5,0x00A6,0x00A7,0x00BD,0x00BE,0x00B1,0x00B2,0x00B3,0x00B4,0x00B5]),
    ("Rétroéclairage",[0x7800,0x7801,0x7802,0x7803,0x7804,0x7805,0x7806]),
    ("Spéciales",     [0x0000,0x0001]),
]

def load_custom_keycodes(def_path):
    """Inject Keychron customKeycodes (QK_KB_0 + index) from a VIA JSON file."""
    try:
        with open(def_path) as f:
            d = json.load(f)
    except OSError:
        return
    customs = d.get("customKeycodes", [])
    codes = []
    for i, ck in enumerate(customs):
        code = QK_KB_0 + i
...
def label(code: int) -> str:
    return CODE2LABEL.get(code, f"0x{code:04X}")

# auto-load the bundled K5 v2 ISO definition if present
